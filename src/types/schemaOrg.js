            ...
            for (const property of properties) {
                if (Array.isArray(data[property])) {
                    ...
                    for (const [index, element] of data[property].entries()) {
                        const elementIssues = await this.validateSchema(element, `${path}.${this.#stripSchema(property)}[${index}]`);
                        issues.push(...elementIssues);
                    }
                } else if (typeof data[property] === 'object' && data[property] !== null && data[property]['@type']) {
                    const propertyIssues = await this.validateSchema(data[property], `${path}.${this.#stripSchema(property)}`);
