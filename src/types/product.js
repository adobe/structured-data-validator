...
export default class ProductValidator extends BaseValidator {
    ...
    aggregateRating(data) {
        ...
        return new AggregateRatingValidator(this.dataFormat, true).validate(data.aggregateRating);
    }
    ...
    review(data) {
        ...
        if (Array.isArray(data.review)) {
            return data.review.map(r => new ReviewValidator(this.dataFormat, true).validate(r)).flat();
        }
